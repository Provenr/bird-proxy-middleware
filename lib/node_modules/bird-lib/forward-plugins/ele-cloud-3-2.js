...
var mimicUUAPLogin = function (serverInfo) {
  ...
  //保证路径完整
  TARGET_SERVER = getTargetServerUrl(serverInfo);
  ...
  /**
   * method 大象最新加密 2019-12-11 15:00:54.
   * @param {String} word 需要加密字符.
   * @return {String} return 加密后的字符.
   */
  function encrypt(word){
    var key = CryptoJS.enc.Utf8.parse("abcdefgabcdxhy12");
  ...
  var tryUUAPLogin = function (body, accessResponse) {
    ...
    request.post({
      url: SSO_LOGIN_URL,
      json: {
        dxhyu: encrypt(username),
        dxhyp: encrypt(password),
      ...
      try {
        ...
        if (body.code == 500) {
          ...
          // forEach 存在多个cookie的可能
          presentCookie.split(';').forEach(cookie => {
            jar.setCookie(cookie, TARGET_SERVER);
          })
        ...
        birdUtils.logs(['error', 'plugin 3.0.* 版本存在异常，大象的登录又换了？！！！'],
          ['error', body],
          [])
        loginEnded()
        return
...
module.exports = function (isRetry, serverInfo, urlOptions) {
  ...
  if (!cookie || isRetry) {
    ...
    var cookieForUrl = cookie.getCookieString(TARGET_SERVER ? TARGET_SERVER: targetUrl);
